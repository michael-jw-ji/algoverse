...
class Graph:
    ...
    def from_model(cls, model_or_config):
        ...
        if hasattr(model_or_config, 'cfg'):
            ...
            hf_cfg = model_or_config.config
            nkv_heads = getattr(hf_cfg, "n_key_value_heads", getattr(hf_cfg, "num_attention_heads", None))
            if nkv_heads is None:
                nkv_heads = getattr(hf_cfg, "num_attention_heads", None)
    
            graph.cfg = {
                "n_layers": getattr(hf_cfg, "num_hidden_layers", None),
                "n_heads": getattr(hf_cfg, "num_attention_heads", None),
                "parallel_attn_mlp": False,  # HF models usually don't have this
                "n_kv_heads": nkv_heads,
            }
        elif isinstance(model_or_config, dict):
            graph.cfg = model_or_config
        else:
            raise ValueError("Unsupported model/config type passed to from_model")
    
        # Now validate that none of the values are None
        for key, val in graph.cfg.items():
            if val is None:
                raise ValueError(f"Config field {key} is None. Please check your model config.")
        ...
        for layer in range(graph.cfg["n_layers"]):
            ...
            if graph.cfg.get("parallel_attn_mlp", False):
        ...
        assert (
            (graph.cfg["n_layers"] - 1) * heads_per_layer + max_index_needed < graph.n_backward
    ...
    def edge_matrices(self):
        ...
        for edge in self.edges.values():
            edge_scores[
                self.forward_index(edge.parent, attn_slice=False),
                self.backward_index(edge.child, qkv=edge.qkv, attn_slice=False),
            ] = float(edge.score)
            edges_in_graph[
                self.forward_index(edge.parent, attn_slice=False),
                self.backward_index(edge.child, qkv=edge.qkv, attn_slice=False),
    ...
    def from_pt(cls, filename):
        ...
        assert all(
            [
                k in d.keys()
                for k in ["cfg", "src_nodes", "dst_nodes", "edges", "edges_in_graph"]
            ]
        ), f"Bad torch circuit file format. Found keys - {d.keys()}, missing keys - {set(['cfg', 'src_nodes', 'dst_nodes', 'edges', 'edges_in_graph']) - set(d.keys())}"
        assert (
            d["edges"].shape
            == d["edges_in_graph"].shape
            == (len(d["src_nodes"]), len(d["dst_nodes"]))
